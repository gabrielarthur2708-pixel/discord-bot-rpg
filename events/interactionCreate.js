  ...
  async execute(interaction, client) {
    if (interaction.isChatInputCommand()) {
      ...
      if (!command) {
        ...
        return interaction.reply({ content: `❌ Comando \`/${interaction.commandName}\` não está carregado.`, ephemeral: true }).catch(()=>{});
      }
      try { await command.execute(interaction, client); }
      catch (err) {
        console.error(`Erro ${interaction.commandName}:`, err);
        const r = { content: `❌ Erro ao executar \`/${interaction.commandName}\`: ${err.message || 'desconhecido'}`, ephemeral: true };
